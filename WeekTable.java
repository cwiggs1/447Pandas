		...
		for (int j = 0; j < model.getRowCount(); j++) {
			...
			for (int i = cal.get(Calendar.DAY_OF_WEEK); i <= Calendar.SATURDAY; i++) {
				...
				if (j == 0) {
					...
					model.setValueAt(sched.shifts.get(weeksApart * 19 + dayOffset + j).getEmploy_id(), j, i - 1);	
				} else if (i == 2 && (j == 1 || j == 7)) {
					//model.setValueAt(aValue, i, j);
				} else if (j == 3) {
					if (i == 1 || i == 7) { 
						model.setValueAt(sched.shifts.get(weeksApart * 19 + dayOffset + (j - 1)).getEmploy_id() , j, i - 1);
					} else {
						model.setValueAt(sched.shifts.get(weeksApart * 19 + dayOffset + j).getEmploy_id(), j, i - 1);
					}
				}
